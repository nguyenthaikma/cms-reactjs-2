...
function TaxonomyDetail({ postType, pageTitle }: TTaxonomyDetail) {
  ...
  } = useQueryTaxonomyMakeTree(postType, token)
  ...
  return (
    <>
      ...
      <Col span={24}>
        <FormSidebar form={form} isLoading={!!isLoadingTaxonomy} scrollToFirstError onFinish={onFinish}>
          <>
            <FormSidebar.Content>
              <Card
                hoverable
            ...
            <FormSidebar.Sidebar>
              <ActionPublish data={{}} loadingUpdate={isLoadingPatchTaxonomyById} onUpdate={() => form.submit()} />
            </FormSidebar.Sidebar>
          </>
        </FormSidebar>
      </Col>
